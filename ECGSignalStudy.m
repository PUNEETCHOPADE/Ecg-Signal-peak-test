...
ylabel('Amplitude (mV)')
title('ECG signal')
xlim([21,31]) % Used to zoom in on single ECG waveform
...
for k = 2 : length(y1)-1
    %the peak has to be greater than 1 and greater than the value before it and greater then the value after it.
   if(y1(k)> y1(k-1) && y1(k) > y1(k+1) && y1(k)> 200) 
        beat_count = beat_count +1;
    end        
end
display (k);
...
[~,locs_Swave] = findpeaks(ECG_inverted,'MinPeakHeight',0.4,'MinPeakDistance',120);                                                            
...
[~,locs_Twave] = findpeaks(ECG_data,'MinPeakHeight',-0.02,'MinPeakDistance',40);
